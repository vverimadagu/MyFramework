...
public class CustomReport extends CustomReportListener {
	 ...
	 * ------------------------------------------------ private String
	 * applicationURL =
	 * "http://COAUWEBTST506:90/Account/Login?ReturnUrl=%2f";//Constants
	 * .applicationURL; private String applicationUserID
	 * ="Constants.applicationUserName"; private String applicationPassword =
	 * "Password1";//Constants.applicationPassword;
	 * 
	 * private String databaseURL ="";// Constants.databaseURL; private String
	 * databaseUserID ="";// Constants.databaseUserName; private String
	 * databasePassword ="";// Constants.databasePassword;
	 */
	// ~ Instance fields ------------------------------------------------------
	...
	public void generateReport(List<XmlSuite> xml, List<ISuite> suites,
		...
		try {
			...
			m_out = createWriter(System.getProperty("user.dir")
					+ "\\custreports");
		} catch (IOException e) {
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		return new PrintWriter(new BufferedWriter(new FileWriter(new File(
				outdir, "BHAutomationReport.html"))));
	}
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				summaryCell(formatter
						.format((overview.getEndDate().getTime() - overview
	 ...
	 * m_out.println("<b>Execution Parameters</b>"); tableStart("testOverview",
	 * null); m_out.print("<tr>"); tableColumnStart("AppicationURL");
	 * tableColumnStart("AppUserName|Password");
	 * //tableColumnStart("DatabaseURL");
	 * //tableColumnStart("DBUserName|Password"); m_out.println("</tr>");
	 * summaryCell(applicationURL, true); summaryCell(applicationUserID + "|" +
	 * applicationPassword, true); summaryCell(databaseURL, true);
	 * summaryCell(databaseUserID + "|" + databasePassword, true);
	 * m_out.println("</table>"); m_out.println("<p></p>"); }
	 */
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println("<head>");
		out.println("<title>BH Automation Execution Report</title>");
		...
		out
				.println(".failedeven td,.stripe .numi_attn {background-color: #D00}");
...
/*
 * With these steps you already have your listener ready to listen.
 * 
 * How to call it?
 * 
 * If you use testng.xml add the following lines:
 * 
 * <listeners> <listener class-name='[your_class_path].CustomReportListener'/>
 * </listeners> If you run your tests from a java class, add the following
 * lines:
 * 
 * private final static CustomReportListener crl = new CustomReportListener();
 * TestNG testng = new TestNG(); testng.addListener(crl); With these steps, when
 * you execute your tests you�ll have two emailable reports, customized and
 * original.
 * 
 * 0 COMMENTSLEAVE A COMMENT JANUARY 16, 2014 TESTAUTOMATIONENGR AUTOMATION,
 * TEST AUTOMATION, TESTNG, TESTNG LISTENERS, TESTNG REPORT How
 */