 ...
 *
 */
public class REPORTER extends INITIALIZE{
	...
	public static String QcentralURL = "http://qcentral-e2e.qwest.net:8001/portal/site/qcentral/template.LOGIN/";
	...
	public static void LogEvent(TestStatus Status, String Expected,
		...
		case FAIL: {
			try {
				...
				//Click Logout
				if(COMMON_METHODS.isElementPresent("Logout", "linkText")){
...
/*	protected static long currentTimeInMillis() {
	 ...
	 * From, To, Subject, Message, attachment path and CC AUTHOR: Dixit Adoor
	 * (dadoor) CREATION DATE: Nov 01 2011
	 * ***************************************
	 * ************************************************
	 */
	/*public static void sendMail(String from, String to, String subject,
			String message, String attachpath, String addto) {
		try

		{
			// String[] filenames = { "TEST REPORT" };
			MailMessage mailMsg = new MailMessage(from, to, subject, message, "mailgate.uswc.uswest.com");
			
			// attachments
			ArrayList<String> attch = new ArrayList<String>();
			attch.add(attachpath);
			mailMsg.setAttachments(attch);
			
			
			// // also test external mail
			mailMsg.addTo(addto);
			
			
			MailUtils.sendMail(mailMsg);
			System.out.println("\n*********");
			System.out.println("\nSent Mail");
			...
			if (vException.indexOf("(") != -1) {
				vException = vException.substring(0,
						vException.indexOf("(") - 1);
				writeToNotepad("Fail", "Unable to send email - ",
...
*/	public static void takeScreenShot(String status) throws Exception {
		...
		//ScreenShotFile = getTestData("TD_ENV")+"_"+getTestData("TD_Script_Name");
	...
	public static void catchException(Exception e, String Expected) {
		Selenium selenium = new WebDriverBackedSelenium(COMMON_METHODS.driver, QcentralURL);
